...
const RONIN_STATE = {
    ...
    DASHING: 'DASHING',
    HURT: 'HURT',
    WAITING_EXECUTION: 'WAITING_EXECUTION', // Boss ajoelhado
    EXECUTED: 'EXECUTED', 
    DEAD: 'DEAD'
};
...
export class Level4Scene extends Phaser.Scene {
    ...
    updateBossBehavior(time, delta) {
        ...
        const distX = Math.abs(this.player.x - this.boss.x);
        
        // 1. Só vira para o player se ele não estiver exatamente em cima (distX > 8)
        ...
        // --- PUNIÇÃO DE CURA (SÓ NA ÚLTIMA VIDA) ---
        ...
        switch (this.boss.state) {
            ...
            case RONIN_STATE.CHASE:
                ...
                const distX = Math.abs(this.player.x - this.boss.x); // Nova variável para a zona neutra

                // VELOCIDADE DINÂMICA POR FASE
                let speed = 350; 
                if (this.boss.currentLives === 2) speed = 400; 
                if (this.boss.currentLives === 1) speed = 480; 

                // Se o player estiver quase em cima dele (até 15px de distância X), ele para
                if (distX <= 15) {
                    this.boss.setVelocityX(0);
                    this.boss.play('ronin_idle', true);
                } else {
                    // Se estiver longe o suficiente, ele persegue
                    this.boss.setVelocityX(dir * speed);
                    this.boss.play('ronin_run', true);
                }
                
                // As condições de troca de estado continuam funcionando
                if (dist < 120) { 
    ...
    decideAttack(dist) {
        ...
        // Distância Curta (< 150px)
            ...
            // Verificação de Cooldown (10 Segundos)
    ...
    performLightningDash() {
        ...
        this.boss.once('animationcomplete', () => {
            ...
            const dist = Phaser.Math.Distance.Between(this.player.x, this.player.y, this.boss.x, this.boss.y);
            if (dist < 220) {
                // Verifica Fase antes de soltar especial pós-dash
                if (Math.random() < 0.6) {
                    this.performCombo();
                } else {
    ...
    performSpecial() {
        ...
        const spListener = (anim, frame) => {
            ...
            if (frame.index <= 8) {
                ...
                if (!hasCaughtPlayer) {
                    // --- Interrupção de FSM (Command Grab / Agarrão) ---
                    // Este ataque especial utiliza Geometria pura em vez da física padrão.
                    // Se o jogador for apanhado na zona de "grab", o seu FSM é forçado para "CUTSCENE",
                    // retirando-lhe o controlo de inputs e sincronizando a sua animação de dor
                    // com os frames exatos da animação do Boss.

                    const grabWidth = 80; 
                    const grabRect = new Phaser.Geom.Rectangle(hitX - (grabWidth / 2), hitY - 60, grabWidth, 120);
            ...
            if (frame.index === 9) {
                this.boss.clearTint(); 
                const damageRect = new Phaser.Geom.Rectangle(
                    this.boss.x - 260, 
                    this.boss.y, // Começa da linha da cintura/chão do boss
                    520, 
                    80           // Altura bem mais baixa, evitando o hit fantasma na plataforma
                );
                const playerRect = this.player.getBounds();
    ...
    handlePlayerAttackBoss(hitbox, boss) {
        ...
        if (boss.health <= 0) {
            if (boss.currentLives > 1) {
                ...
                // Se a vida acabou de cair para 1, é a hora do show da Fase 3!
                if (boss.currentLives === 1) {
                    
                    this.triggerFase3Surprise(boss);
                    
                } else {
                    // Se a vida caiu para 2, faz a transição normal (Fase 1 para Fase 2)
                    this.updateHUD();
        ...
        // Se o Mestre Jubei JÁ estiver no estado HURT (tomando um combo), 
        // em vez de travar a animação, ele pune o jogador escapando com um Dash
        if (boss.state === RONIN_STATE.HURT) {
            this.showFloatingText(boss.x, boss.y - 50, "Basta!", 500);
            this.performLightningDash(); // Ele foge e já revida
            return; // Interrompe o código aqui para não reiniciar a animação de dor
        }
    ...
    handleBossHitPlayer(hitbox, player) {
        ...
        if (player.isParrying) {
            ...
            // Força o estado HURT para garantir o Stun
            this.setBossState(RONIN_STATE.HURT);
            this.boss.setVelocityX(0);
    ...
    startFlashbackSequence() {
        // --- Manipulação de Tempo e Execução (Cinematografia) ---
        // Em vez de simplesmente tocar uma animação de morte, o Boss entra em espera (WAITING_EXECUTION).
        // A propriedade timeScale do motor é reduzida para 0.1, criando um efeito de "Slow Motion".
        // O jogo aguarda que o jogador dê o golpe final interativo para encerrar a luta,
        // fundindo a narrativa com a própria jogabilidade.
    ...
    triggerBossKneel() {
        ...
        const kneelListener = (anim, frame) => {
            // Se o estado já mudou para EXECUTED (jogador atacou caindo),
    ...
    executeFinalBlow() {
        ...
        // Garante velocidade normal caso tenha atacado durante o slow motion
        this.time.timeScale = 1;
    ...
    createSparks(x, y) {
        // Força a cor amarela nas faíscas de impacto
    ...
    createHUD() {
        ...
        //  Stamina e Cura Copiado do Level 3
    ...
    updateHUD() {
        ...
        if (this.boss && this.bossFightStarted && !this.isFlashbackSequence) {
            ...
            for (let i = 0; i < this.boss.maxLives; i++) {
                if (this.bossLives[i]) {
                    // [MODIFICAÇÃO: Oculta a Fase 3 inicialmente]
                    if (i === 0 && !this.fase3Revelada) {
                        this.bossLives[i].setVisible(false);
                    } else {
    ...
    triggerFase3Surprise(boss) {
        // --- Design Narrativo via HUD (O Falso Final) ---
        // A interface (UI) mente intencionalmente ao jogador escondendo a última vida (Fase 3).
        // Quando o jogador pensa que venceu, desativamos as IAs, acionamos efeitos dramáticos de
        // ecrã (Camera Shake/Flash) e revelamos o verdadeiro desafio (Modo Berserk).

        // 1. Para o combate e desativa as IAs temporariamente para a cutscene
        this.bossAIEnabled = false;
        boss.setVelocity(0, 0);
        boss.play('ronin_defend'); // Coloca o boss em postura imponente
        
        this.player.playerState = 'CUTSCENE'; // Trava o jogador para ele assistir
        this.player.setVelocity(0, 0);
        this.player.anims.play('idle', true);

        // 2. Efeitos visuais impactantes (Flash branco e terremoto na câmera)
        this.cameras.main.flash(800, 255, 255, 255);
        this.cameras.main.shake(1200, 0.03);
        
        // Deixa o boss com uma aura avermelhada de perigo
        boss.setTint(0xff3333);

        // 3. Mensagem intimidadora na tela
        this.showFloatingText(boss.x, boss.y - 80, "A LÂMINA DEVOROU O SEU CORAÇÃO... DEIXE-ME CORTAR O QUE SOBROU!", 4000);

        // 4. Pausa dramática de 2 segundos antes da fúria começar
        this.time.delayedCall(2000, () => {
            this.fase3Revelada = true; // Faz o terceiro círculo aparecer no HUD
            this.updateHUD();          // Força a atualização visual da barra
            
            // 5. Devolve o controle e inicia o Modo Berserk
            this.bossAIEnabled = true;
            this.player.playerState = 'IDLE';
            boss.clearTint();
            
            // Começa a fase nova já com o Lightning Dash agressivo na cara do player
            this.performLightningDash();
        });
    }
