...
export class Level1Scene extends Phaser.Scene {
    ...
    create() {
        try {
            ...
            // Camadas mais ao fundo recebem um scrollFactor menor, fazendo com que 
            // se movam mais devagar em relação à câmera, gerando a ilusão de profundidade.
    ...
    startIntroCutscene() {
        ...
        this.introEvent = this.time.addEvent({
            ...
            callback: () => {
                ...
                // --- Transição Cutscene para Gameplay ---
                // Verifica se a animação roteirizada atingiu o ponto alvo (x >= 180).
                // Ao atingir, restauramos a física do jogador (gravidade e colisões)
                // e devolvemos o controle de estado para o usuário.
    ...
    update(time, delta) {
        ...
        if (Phaser.Input.Keyboard.JustDown(this.keyR)) {
            this.scene.start(this.scene.key); // Melhor que restart() direto para resetar estado
            return;
        }
    ...
    handleDoorInteraction() {
        ...
        // --- Interação Dinâmica (Data-Driven) ---
        // Lê as propriedades customizadas dos tiles configuradas no software Tiled.
        // Isso evita o hardcoding de coordenadas. Se o Level Designer mover a porta
        // no mapa, o código continua funcionando perfeitamente procurando pela prop "door".
