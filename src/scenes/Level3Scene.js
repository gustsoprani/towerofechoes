...
export class Level3Scene extends Phaser.Scene {
    ...
    updateBossBehavior(time, delta) {
        ...
        // A IA da Akane é controlada por uma Máquina de Estados rigorosa.
        // O Switch garante que ela só execute a lógica pertencente ao seu estado atual
        // (ex: não pode decidir andar enquanto está no meio de um ataque ou recebendo dano),
        // tornando o código escalável e livre de "race conditions" nas animações.
        switch (this.boss.state) {
            ...
            case BOSS_STATE.CHASE:
                ...
                const distX = Math.abs(this.player.x - this.boss.x); // Calcula a distância X

                // Se estiver muito próxima no eixo X, para de andar e fica idle
                if (distX <= 20) {
                    this.boss.setVelocityX(0);
                ...
                // Mantém as condições de transição
                if (dist < 120) { 
    ...
    performSkill() {
        ...
        const skillListener = (anim, frame) => {
            ...
            // A hitbox esta nos frames (4 a 8), 
    ...
    handleAttackHitEnemy(hitbox, enemy) {
        ...
        // --- Esquiva Preventiva (Reatividade da IA) ---
        // Se a Akane não estiver presa em uma animação de ataque (estiver Idle ou Chase),
        // ela possui uma chance (RNG) de perceber o golpe do jogador e desviar
        // antes que a hitbox de dano seja computada. Isso força o jogador a criar aberturas.
        ...
        if (enemy.health <= 0) {
            if (enemy.currentLives > 1) {
                ...
                // Reseta IA
                this.setBossState(BOSS_STATE.IDLE); 
                this.performBackDodge(); // Foge para reiniciar
                return;
            } else {
                // --- MORTE ---
                this.killBoss(enemy);
        ...
        // --- Super Armor (Resistência a Interrupção) ---
        // Se a chefe estiver no meio de um ataque, ela recebe o dano mas NÃO é atordoada
        // (ignora o estado HURT). Isso pune jogadores que tentam trocar dano de forma imprudente.
        ...
        // --- Combo Breaker ---
        // Se ela já estiver no estado HURT e receber outro golpe rápido,
        // ela quebra o combo do jogador recuando imediatamente.
    ...
    killBoss(enemy) {
        ...
        // 2. Congela o Player no frame exato (espada levantada)
