...
export class Level2Scene extends Phaser.Scene {
    ...
    create() {
        try {
            ...
            // --- Arquitetura de IA (Boolean Flags) ---
            // Ao contrário dos chefes finais que utilizam Finite State Machines (FSM),
            // o Varg utiliza um sistema de controle baseado em Flags Booleanas.
            // Essa abordagem atende bem a padrões de ataque mais rígidos e scriptados.
        ...
        this.time.addEvent({
            ...
            callback: () => {
                ...
                if (part) {
                    ...
                    // --- Efeito Visual Complexo (Curvas de Bézier) ---
                    // Em vez de uma interpolação linear simples (reta), calculamos um 
                    // vetor de controle (controlPoint) aleatório para gerar uma curva 
                    // quadrática. Isso cria o efeito orgânico das almas sendo sugadas.
    ...
    bossAIUpdate() {
        ...
        if (boss.isEnraged) {
            ...
            // --- Reatividade de Combate (Estus Punish) ---
            // A IA do chefe monitora o estado do jogador. Se detectar uma tentativa de cura,
            // ignora o cooldown normal e aciona um teletransporte punitivo quase garantido,
            // forçando o jogador a procurar aberturas seguras para se curar.
        ...
        const distX = Math.abs(this.player.x - boss.x); // Nova variável de zona neutra

        // Verifica se ele está perto demais (zona neutra)
        if (distX <= 25) { 
            boss.setVelocityX(0);
            boss.play('nb_idle', true);
        } 
        // Se estiver longe, persegue
        else if (dist > 250 || (dist > 150 && !boss.canStab)) {
